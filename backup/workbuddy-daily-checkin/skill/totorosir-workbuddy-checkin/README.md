...
**Q：签到会不会消耗积分？怎么选模型最省？**
A：签到脚本本身只发 HTTP 请求，**不消耗积分**；真正花钱的是「跑这个任务的模型」。选用顺序固定为两步：
① **优先免积分模型**（客户端模型列表里标注「限时免费」的，如混元系 `Hy3`）；
② 免积分不可用时，**选倍率最低的模型**（当前 `Deepseek-V4-Flash`，倍率 x0.06，以客户端当期标注为准）。
**别留 `Auto`**——它按「能力最优」而非「价格最省」调度，倍率可能高出几十倍。签到属于「跑一条命令 + 汇报一句话」的轻任务，用最便宜的模型完全够。
设置入口：**自动化 → 编辑该任务 → 选择模型和技能**（模型是任务级配置，脚本改不了，需你手动选定）。

**Q：电脑关机了，签到还能跑吗？能不能把它放到云端跑？**
A：**放到云端跑不通**，这是原理问题不是配置问题：签到的命脉是**你电脑上的 WorkBuddy 登录态文件**，云端沙箱是另一台机器、另一个文件系统，没有这个文件；把 token 抄上去既等于把凭据搬出本机，也躲不过 token 过期（刷新只能由桌面端做），迟早静默失效。
另外两个容易误判的点：① WorkBuddy 的「云端任务 / 小程序云上模式 / 项目」确实能 7×24 不用开机，但它们拿不到你桌面端的登录态；② OS 级「定时开机」（`pmset`）在**开了 FileVault** 的 Mac 上只会通电到解锁界面，系统没起来，WorkBuddy 不会运行。
真正可行的三条路：**① 换一台常开设备**（Mac mini / 云主机 / 旧笔记本）在那台机器登录并重建同一套自动化——唯一能真 7×24 的方案；**② 用满补跑窗口**：任务定 15:00，12h 窗口到次日 03:00，当天 03:00 前开过机就会补签；**③ 尽量用「睡眠」代替「关机」**——睡眠不算关机，唤醒后会自动补扫。
...
- ❌ **不要用 `Auto` 或高倍率模型跑签到任务**：签到是纯脚本轻任务，`Auto` 按能力最优调度，倍率可能高出几十倍；按「免积分优先、其次最低倍率」选模型即可。
